        SUBROUTINE UMZ2F0 (N, NZ, CP, XX, XSIZE, II, ISIZE, XTAIL,
...
C       ne:             number of triplets, unchanged from UMZ2FA
...
C       Keep (6..8):    integer control parameters, see UMZ21I
C
C       if presrv is true:
...
C=======================================================================
C
C       called by subroutine:   UMZ2FA
C       subroutines called:     UMZ2ER, UMZ2FB, UMZ2F1, UMZ2OF
...
C  iout:    true if UMZ2F1 ran out of integer memory, but did not
C           set error flag
C  xout:    true if UMZ2F2 ran out of integer memory, but did not
C           set error flag
C
C  Estimated memory for UMZ2RF:
C  ----------------------------
C  rmax:    largest contribution block is cmax-by-rmax
C  cmax:       "         "         "    "   "   "  "
C  totnlu:  total number of LU arrowheads in all diagonal blocks
C  xrmax:   estimated maximum real memory usage for UMZ2RF
C  xruse:   estimated current real memory usage for UMZ2RF
C
C  Other:
C  ------
...
***  XOUT and IOUT need to be initialized.  They are set in UMZ2FB, but that ***
...
C                error return, if not enough integer memory:
                 GO TO 9000
              ENDIF 
              CALL UMZ2FB (AX, ANZ, AI, ANZ, N, NZ, NZDIA, NZOFF,
...
C                error return, if not enough integer and/or real memory:
                 GO TO 9000
              ENDIF 
              CALL UMZ2FB (XX, 2*NZ, II, 2*NZ, N, NZ, NZDIA, NZOFF,
     $              NBLKS, CP, II (CPERMP), II (RPERMP), II(P), II(P+N),
     $              II (P+2*N), II (P+3*N), II (P+4*N), II (OFFPP),
     $              PRESRV, ICNTL)
...
C          -------------------------------------------------------------
           ...
           CALL UMZ2F1 (CP, N, II (CPERMP), II (RPERMP), NZOFF,
...
C             error return, if not enough integer and/or real memory:
           ...
           ENDIF 
           IF (INFO (1) .LT. 0) THEN 
C             error return, if error in UMZ2F2:
              GO TO 9010
           ENDIF 
...
C          -------------------------------------------------------------
           ...
           IF (NBLKS .EQ. N) THEN 
C             otherwise, arrays in UMZ2RF and UMZ2SO would have
...
C          use Cp as temporary work array:
           MNZ = NZOFF
           IF (PRESRV) THEN 
              CALL UMZ2OF (CP, N, II (RPERMP), II (CPERMP), NZOFF,
...
C       error return label, for error from UMZ2F2:
9010    CONTINUE
...
C          set warning flag if matrix is singular
           CALL UMZ2ER (1, ICNTL, INFO, 4, INFO (17))
        ENDIF 

C       ----------------------------------------------------------------
C       Determine an upper bound on the amount of integer memory needed
C       (LINDEX) for a subsequent call to UMZ2RF.  If block-upper-
C       triangular-form is not in use (Info (9) is 1), then
C       this bound is exact.  If NE is higher in the call to UMZ2RF
C       than in the call to UMZ2FA, then add 3 integers for each
...
C       in UMZ2FA, to complete the computation of the estimate.
...
C       subsequent call to UMZ2RF.  The approximation is an upper bound
C       on the bare minimum amount needed.  Some garbage collection may
C       occur, but UMZ2RF is guaranteed to finish if given an LVALUE of
C       size Info (23) and if the pattern is the same.  If NE is
C       higher in the call to UMZ2RF than in the call to UMZ2FA, then
