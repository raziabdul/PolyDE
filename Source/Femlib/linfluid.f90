...
!*******************************************************************************
!
!  read in the solution vector from file solution generated by FLUID solver.
!
!*******************************************************************************
! remarks: read multiple solution vectors of Ux, Uy, P, T, stream function and
!          density (if compressible flow is solved).
!
!  Input:
...
!  local variables
!*******************************************************************************
      integer (I4B) unitid, i, j, neldof, nstored
      ...
      logical exist, exist_t, exist_str
!
      external    ::    grglun
!
      ...
      inquire (file=path(1:len_trim(path))//'solution_str',exist=exist_str)
     ...
     &         form='unformatted',status='old',position='REWIND',action='READ')
      ...
      if (exist_str) then
         call grglun(unitid3)
         open (unitid3,file=path(1:len_trim(path))//'solution_str',                 &
     &         form='unformatted',status='old',position='REWIND',action='READ')
      end if
...
!
            ...
            if (exist_str) then
               allocate(xfluid(5,ndof))
            else if (exist_t) then
...
!  read in the solution (stored for each DOF)
            do j=1,ndof
               read (unitid,end=1000,err=1000) (xfluid(i,j),i=1,3)
            end do
            ...
            if (exist_str) then
               ...
               do j=1,ndof
                  read (unitid3,end=1000,err=1000) xfluid(5,j)
               end do
            end if
!
         else
     ...
     &        dayofweek,day,month,year,hour,minute,second
      ...
      if (exist_str) close (unitid3,err=1000)
!
      ok=.true.
      return
