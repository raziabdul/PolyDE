      subroutine assembly(lower,upper,diag,acsr,rhs,ia,ja, &
      &                   jacobi,csr,matvar,symmetric)
      use feminterface, only: elementmatrix, zeit, reallocate, destroyarrptr, coloring
      use femtypes
      use globalvariables, only: n, ndof, nnat, ep, eg
      implicit none
      integer (I4B), pointer :: ia(:), ja(:)
      complex (DPC), pointer :: lower(:), upper(:), diag(:), acsr(:), rhs(:)
...
!  indices iloop2 and iloop4 take care of the multinature LCSR assembly for multinature symmetric matrices
      integer (I4B) iloop2, iloop4, errcode, vzk, vzl
      integer (I4B) count
      integer (I4B), dimension(:), allocatable :: nff, nffsum
      complex (DPC), pointer :: ai(:,:), bi(:)
...
!  clear matrix, right-hand-side
        allocate(acsr(1:ia(ndof+1)-1), rhs(ndof))
        acsr=(0._DP,0._DP)
        rhs=(0._DP,0._DP)
!
        ncolor=1                !serial
...
!
!  loop over all elements (loop in parallel, range given by color/nocolor loop)
! need to allocate here, not in elementmatrix as before
        do elem=cstart,cend
!
          i=elem            !serial
!
...
!  element matrix to be computed for all natures, argument ep(i,:)
          allocate( nff(nnat) )
          allocate( nffsum(nnat+1) )  
          call elementmatrix(i,ep(i,:),jacobi,.true.,matvar,ai,bi,nff,nffsum,errcode)
!  loop over all natures
          do inat=1,nnat
            do k=1,size(eg(i,inat)%d)
!  row index k: local;  kn: global
...
!  compute element matrix
! allocate to avoid error from oneAPI
          allocate( nff(nnat) )
          allocate( nffsum(nnat+1) )  
          call elementmatrix( i, ep(i,:), jacobi, .true., matvar, ai, bi, nff, nffsum, errcode )
!  loop over all natures
