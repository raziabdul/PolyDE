...
!
!
!  Computation of the element matrices s, d and m for a single element
!  using numerical integration (according to Su + D(du/dt) + M(d2u/dt2) = b)
!
...
!
!  Output:   s        stiffness matrix  or  Jacobian matrix
!            d        damping matrix
!            m        mass matrix
...
!  pde coefficients
      ...
      complex (DPC) beta(2,nnat,nnat), em(nnat,nnat), f1(nnat), f2(2,nnat)
!  weighted pde coefficients
      complex (DPC) nuaw(2,2), gammaaw(2), betaaw(2)
      complex (DPC) alphaaw, emaw, f1aw
      complex (DPC) pp(nnat), qq(nnat,nnat), value, q1, q2, q3, q4, q5, q6, q7
...
!       which offers the advantage of evaluating all formfunctions at the same locations 
!       thus reducing the work required in the evaluation of formfunction
...
!  require embedded (eventually adaptive) Gaussian integration formulae over the triangle.
!
...
!  Allocate required space and initialise elementmatrices s, d and m, and RHS b
      allocate(s(sum(nff),sum(nff)), d(sum(nff),sum(nff)), m(sum(nff),sum(nff)), b(sum(nff)),stat=errcode )
      s(:,:)=(0._DP,0._DP)
      d(:,:)=(0._DP,0._DP)
      m(:,:)=(0._DP,0._DP)
      b(:)=(0._DP,0._DP)
      allocate(xsi(maxval(nff)), gxsi(maxval(nff),2),stat=errcode )
!
...
!  beta is a vector
              q3 = betaaw(1)*gxsi(j,1)+betaaw(2)*gxsi(j,2)
!  alpha is a scalar
              q4 = alphaaw*xsi(j)
!  em is a scalar
              q5 = emaw*xsi(j)
...
!
!
                s(i+nffsum(inat),j+nffsum(jnat)) = s(i+nffsum(inat),j+nffsum(jnat))     &
           &    + q1*gxsi(i,1) + q2*gxsi(i,2) + q3*xsi(i)

                d(i+nffsum(inat),j+nffsum(jnat)) = d(i+nffsum(inat),j+nffsum(jnat))     &
           &    + q4*xsi(i)

                m(i+nffsum(inat),j+nffsum(jnat)) = m(i+nffsum(inat),j+nffsum(jnat))     &
           &    + q5*xsi(i)
!
!  contributions to the jacobian matrix if nonlinear
!  ## to be implemented
!            if (jacobi) then
!              s(i,j)=s(i,j)+weight(k)*(whatjacobi)
!            end if
              end do
!
...
!  f_1*xsi_i-f_2*grad(xsi_i)
              if (inat .eq. jnat) then
                b(j+nffsum(jnat)) = b(j+nffsum(jnat)) + f1aw*xsi(j) - q6*gxsi(j,1) - q7*gxsi(j,2)
              end if
!
            end do
          end do
...
!  check whether it is a general or Neumann BC
     ...
     &        zrb(brancheg(iedge,inat),inat).ge.300) cycle
              call getbcval2D(brancheg(iedge,inat),inat,xs,ys,pp(inat),qq(inat,:))
...
!  we may cycle here if the shape function xsi_j is zero at this edge
                   s(j+nffsum(jnat),i+nffsum(inat))=s(j+nffsum(jnat),i+nffsum(inat)) - &
